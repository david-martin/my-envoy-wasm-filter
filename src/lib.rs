...
impl HttpContext for MyHttpFilter {
    ...
    fn on_http_response_body(&mut self, body_size: usize, end_of_stream: bool) -> Action {
        let _ = proxy_wasm::hostcalls::log(
            LogLevel::Info,
            "Wasm filter: on_http_response_body called!",
        );
        if let Some(chunk) = self.get_http_response_body(0, body_size) {
            let _ = proxy_wasm::hostcalls::log(
                LogLevel::Info,
                &format!(
                    "Wasm filter: Got response body chunk! size: {:?}",
                    &chunk.len()
                ),
            );
        ...
        if end_of_stream {
            ...
            if let Ok(body_str) = std::str::from_utf8(&self.response_body) {
                let _ = proxy_wasm::hostcalls::log(
                    LogLevel::Info,
                    &format!("Wasm filter: Got body string! str: {:?}", body_str),
                );
                if let Ok(json_val) = serde_json::from_str::<Value>(body_str) {
                    let _ = proxy_wasm::hostcalls::log(
                        LogLevel::Info,
                        &format!("Wasm filter: Got json value! json_val: {:?}", json_val),
                    );
                    if let Some(usage_val) = json_val.get("usage") {
                        let _ = proxy_wasm::hostcalls::log(
                            LogLevel::Info,
                            &format!("Wasm filter: Got usage value! usage_val: {:?}", usage_val),
                        );
                        if let Ok(parsed) = serde_json::from_value::<Usage>(usage_val.clone()) {
                            let _ = proxy_wasm::hostcalls::log(
                                LogLevel::Info,
                                &format!("Found usage: {:?}", parsed),
                            );

                            // Increments must be i64 (not u64)
                            let _ =
                                increment_metric(self.metric_prompt_tokens, parsed.prompt_tokens);
